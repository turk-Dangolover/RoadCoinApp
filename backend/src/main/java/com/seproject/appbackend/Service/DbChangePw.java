...
public class DbChangePw {
    ...
    public boolean changeUserData(String verification_id, String oldPassword, String newPassword) {
        try {
            String checkPasswordSql = "SELECT COUNT(*) FROM users WHERE verification_id = ? AND password = ?";
            @SuppressWarnings("deprecation")
            int count = jdbcTemplate.queryForObject(checkPasswordSql, new Object[]{verification_id, oldPassword}, Integer.class);

            if (count == 1) {
                String updatePasswordSql = "UPDATE users SET password = ? WHERE verification_id = ?";
                jdbcTemplate.update(updatePasswordSql, newPassword, verification_id);
                return true;
            } else {
                return false;
            }
